...
SELECT Count(*) AS movies_with_one_genres 
         FROM (SELECT movie_id
         FROM  genre
         GROUP  BY movie_id
         HAVING Count(DISTINCT genre) = 1) AS Sub;
...
-- Displaying the top 10 movies using LIMIT clause & Finding the rank of each movie based on it's average rating
SELECT     title,
           avg_rating,
...
-- Finding the rank of production company based on movie count with average rating > 8 using RANK function & 
-- find the production company with rank=1.
...
-- Query to check if Germany votes > Italian votes using SELECT IF statement
-- Answer is YES if Germany votes > Italian votes
-- Answer is NO if Germany votes <= Italian votes
...
Country_Vote AS
(
SELECT country FROM VOTES_SUMMARY
ORDER BY total_votes DESC
LIMIT 1)
SELECT IF (country LIKE 'GERMANY' , 'YES', 'NO') AS ANSWER
FROM Country_Vote ;
...
-- Computes the top 3 genres using average rating > 8 condition and highest movie counts
-- The directors are found whose movies have an average rating > 8 and are sorted based on number of movies made by using the top genres  
